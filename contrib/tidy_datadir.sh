...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Sedus database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Sedus datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Sedus datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Sedus 0.7 datadir."
    ;;
  3)
    echo "Detected Sedus pre-0.8 datadir."
    ;;
  4)
    echo "Detected Sedus 0.8 datadir."
    ;;
esac
