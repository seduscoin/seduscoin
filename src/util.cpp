...
const char * const BITCOIN_CONF_FILENAME = "sedus.conf";
const char * const BITCOIN_PID_FILENAME = "sedusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sedus" is a composite category enabling all Sedus-related debug output
            if(ptrCategory->count(string("sedus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SedusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SedusCore
    // Mac: ~/Library/Application Support/SedusCore
    // Unix: ~/.seduscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SedusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SedusCore";
#else
    // Unix
    return pathRet / ".seduscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sedus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sedus.conf
